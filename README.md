...
URL du site : https://throne-lord-edition.netlify.com
...
| Type  | Objectif | Fait ? | 
| ----- | -------- | ------ |
| Setup | Le repository respecte la nomenclature `w1p2021-hero-amsellem-calou`, avec tous les noms des membres, et les membres sont cités dans le README.md | OK |
| Setup | Le site est en ligne, et l'URL est citée dans le README.md | OK |
| Setup | Le site utilise VueJS | OK |
| Setup | Cloner le repository et exécuter `npm install` puis `npm run dev` s'effectue sans encombre | OK |
...
| Données | Un fichier `data.json` est utilisé pour stocker la structure du jeu | OK |
| Données | Le fichier `data.json` décrit 30 phases de jeu ou plus. Indiquez *En partie* s'il en décrit plus de 15. | OK |
| Pages | Une page d'accueil est présente | OK |
| Pages | Une page de choix du personnage est présente | OK |
| Pages | Une page de victoire est présente | OK |
| Pages | Une page d'échec est présente | OK |
| Routing | Une route dont l'`id` varie permet d'afficher les différentes étapes du jeu | OK |
| Routing | Lorsqu'on recharge la page (`Ctrl + R`), on se trouve toujours à la même étape | OK |
| Transitions | Chaque page apparaît grâce à une transition fluide (la complexité de la transition n'est pas prise en compte ici) | OK |
...
| État | Au moins un service (classe de type `GameService`) est utilisé | OK |
| Sauvegarde | Lorsqu'on recharge la page (`Ctrl + R`), le personnage, ses caractéristiques, les choix du joueur et tout le reste sont rétablis | OK |
...
| Multimédia | L'expérience présente une vidéo ou plus | OK |
| Multimédia | L'expérience présente un audio ou plus | OK |
| Multimédia | L'utilisateur a la possibilité de couper le son à tout moment | OK |
